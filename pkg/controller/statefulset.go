...
func installInitContainer(
	...
	envList := []core.EnvVar{
		...
		},
		{
			...
			Value: mongodb.Name + ExternalClientSecretSuffix,
		},
		{
			Name:  "SERVER_CERT_NAME",
			Value: mongodb.Name + ServerSecretSuffix,
		},
	}
...
func getExporterContainer(mongodb *api.MongoDB, mongodbVersion *v1alpha1.MongoDBVersion) core.Container {
	...
	// TODO: These arguments are for exporter. older images of mongoDB (ssl generation) might need different arguments
	if mongodb.Spec.SSLMode != api.SSLModeDisabled && mongodb.Spec.TLS != nil {
		clientPEM := fmt.Sprintf("%s/%s", api.MongoCertDirectory, api.MongoClientFileName)
		clientCA := fmt.Sprintf("%s/%s", api.MongoCertDirectory, api.MongoTLSCertFileName)
