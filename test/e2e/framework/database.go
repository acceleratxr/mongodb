...
func (f *Framework) GetMongoDBClient(meta metav1.ObjectMeta, tunnel *portforward.Tunnel, isReplSet ...bool) (*options.ClientOptions, error) {
	...
	if mongodb.Spec.SSLMode == v1alpha1.SSLModeRequireSSL {
		...
		clientOpts = options.Client().ApplyURI(fmt.Sprintf("mongodb://%s:%s@localhost:%v/?ssl=true&sslclientcertificatekeyfile=/tmp/mongodb/%v&&sslcertificateauthorityfile=/tmp/mongodb/%v", user, pass, tunnel.Local, v1alpha1.MongoPemFileName, v1alpha1.MongoTLSCertFileName))
	}
